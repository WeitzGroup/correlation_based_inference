% Supp fig 2 - AUC results for time-delayed spearman and kendall correlation
...
for c = 1:3
    for s = 1:length(S)
        for n = 1:length(N)
            ...
            load(sprintf('data/%s/%sdelayed_S%d_N%d',dir_results_all{c},C{c},s,n));
            ...
            if c==1
                ...
                if n==2 && s==1; title('time-delayed correlation (varying correlation metric)','FontSize',Fs,'FontWeight','normal'); end
...
save_fig(sprintf('%s/suppfig2',save_dir));
