% Supp fig 5 - AUC results for standard (pearson) correlation with
...
for d = 1:length(dir_results_all)
    for s = 1:length(S)
        for n = 1:length(N)
            ...
            load(sprintf('data/%s/pearson_S%d_N%d',dir_results_all{d},s,n));
            ...
            if d==1
                ...
                if n==2 && s==1; title('standard Pearson correlation (varying sampling frequency)','FontSize',Fs,'FontWeight','normal'); end
...
save_fig(sprintf('%s/suppfig5',save_dir));
